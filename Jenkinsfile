            ...
            sh '''#! /bin/bash
                  ...
                  for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                    ...
                    digest=$(curl -s \
                      ...
                      "https://ghcr.io/v2/linuxserver/${CONTAINER_NAME}/manifests/arm32v7-debian-kde")
                    if [[ $(echo "$digest" | jq -r '.layers') != "null" ]]; then
                      docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-debian-kde || :
                      docker manifest create ${MANIFESTIMAGE}:arm32v7-debian-kde ${MANIFESTIMAGE}:amd64-debian-kde
                      docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-debian-kde
                    fi
                    docker manifest push --purge ${MANIFESTIMAGE}:debian-kde
                    docker manifest push --purge ${MANIFESTIMAGE}:${META_TAG} 
