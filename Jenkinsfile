          ...
          retry(5) {
            sh '''#! /bin/bash
                  ...
                  for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                    ...
                    docker manifest create ${MANIFESTIMAGE}:fedora-icewm ${MANIFESTIMAGE}:amd64-fedora-icewm ${MANIFESTIMAGE}:arm64v8-fedora-icewm
                    ...
                    digest=$(curl -s \
                      ...
                      "https://ghcr.io/v2/linuxserver/${CONTAINER_NAME}/manifests/arm32v7-fedora-icewm")
                    if [[ $(echo "$digest" | jq -r '.layers') != "null" ]]; then
                      docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-fedora-icewm || :
                      docker manifest create ${MANIFESTIMAGE}:arm32v7-fedora-icewm ${MANIFESTIMAGE}:amd64-fedora-icewm
                      docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-fedora-icewm
                    fi
                    docker manifest push --purge ${MANIFESTIMAGE}:fedora-icewm
                    docker manifest push --purge ${MANIFESTIMAGE}:${META_TAG} 
