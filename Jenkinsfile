          ...
          retry(5) {
            sh '''#! /bin/bash
                  ...
                  for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                    ...
                    docker manifest create ${MANIFESTIMAGE}:fedora-openbox ${MANIFESTIMAGE}:amd64-fedora-openbox ${MANIFESTIMAGE}:arm64v8-fedora-openbox
                    ...
                    digest=$(curl -s \
                      ...
                      "https://ghcr.io/v2/linuxserver/${CONTAINER_NAME}/manifests/arm32v7-fedora-openbox")
                    if [[ $(echo "$digest" | jq -r '.layers') != "null" ]]; then
                      docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-fedora-openbox || :
                      docker manifest create ${MANIFESTIMAGE}:arm32v7-fedora-openbox ${MANIFESTIMAGE}:amd64-fedora-openbox
                      docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-fedora-openbox
                    fi
                    docker manifest push --purge ${MANIFESTIMAGE}:fedora-openbox
                    docker manifest push --purge ${MANIFESTIMAGE}:${META_TAG} 
