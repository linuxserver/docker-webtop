            ...
            sh '''#! /bin/bash
                  ...
                  for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                    ...
                    digest=$(curl -s \
                      ...
                      "https://ghcr.io/v2/linuxserver/${CONTAINER_NAME}/manifests/arm32v7-debian-xfce")
                    if [[ $(echo "$digest" | jq -r '.layers') != "null" ]]; then
                      docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-debian-xfce || :
                      docker manifest create ${MANIFESTIMAGE}:arm32v7-debian-xfce ${MANIFESTIMAGE}:amd64-debian-xfce
                      docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-debian-xfce
                    fi
                    docker manifest push --purge ${MANIFESTIMAGE}:debian-xfce
                    docker manifest push --purge ${MANIFESTIMAGE}:${META_TAG} 
