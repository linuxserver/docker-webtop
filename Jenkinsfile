          ...
          retry(5) {
            sh '''#! /bin/bash
                  ...
                  for PUSHIMAGE in "${GITHUBIMAGE}" "${GITLABIMAGE}" "${QUAYIMAGE}" "${IMAGE}"; do
                    ...
                    docker tag ${PUSHIMAGE}:${META_TAG} ${PUSHIMAGE}:fedora-mate
                    docker tag ${PUSHIMAGE}:${META_TAG} ${PUSHIMAGE}:${EXT_RELEASE_TAG}
                    if [ -n "${SEMVER}" ]; then
                      docker tag ${PUSHIMAGE}:${META_TAG} ${PUSHIMAGE}:${SEMVER}
                    fi
                    docker push ${PUSHIMAGE}:fedora-mate
            ...
            sh '''#! /bin/bash
                  ...
                  for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                    ...
                    digest=$(curl -s \
                      ...
                      "https://ghcr.io/v2/linuxserver/${CONTAINER_NAME}/manifests/arm32v7-fedora-mate")
                    if [[ $(echo "$digest" | jq -r '.layers') != "null" ]]; then
                      docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-fedora-mate || :
                      docker manifest create ${MANIFESTIMAGE}:arm32v7-fedora-mate ${MANIFESTIMAGE}:amd64-fedora-mate
                      docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-fedora-mate
                    fi
                    docker manifest push --purge ${MANIFESTIMAGE}:fedora-mate
                    docker manifest push --purge ${MANIFESTIMAGE}:${META_TAG} 
